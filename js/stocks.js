...
let monthlyStockVolumeChartInstance = null;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    renderStocks();
    loadStockEvents();
    renderMonthlyStockVolumeChart();

    // Add toggle listener for monthly chart
    const toggleStockChartBtn = document.getElementById('toggle-stock-chart-period');
    if (toggleStockChartBtn) {
        toggleStockChartBtn.addEventListener('click', () => {
            const currentPeriod = toggleStockChartBtn.dataset.period;
            const newPeriod = currentPeriod === 'year' ? 'all' : 'year';
            toggleStockChartBtn.dataset.period = newPeriod;
            toggleStockChartBtn.textContent = newPeriod === 'year' ? 'Show All Time' : 'Show Last Year';
            renderMonthlyStockVolumeChart(newPeriod);
        });
    }
    ...
    portfolio.stocks.forEach(stock => {
        ...
        html += `
            <tr class="border-b border-gray-700">
                ...
                <td class="py-2 px-2">${formatQuantity(stock.quantity)}</td>
...
function renderStockTransactions() {
    ...
    const toggleBtn = document.getElementById('toggle-stock-chart-period');
    const currentPeriod = toggleBtn ? toggleBtn.dataset.period : 'year';
    renderMonthlyStockVolumeChart(currentPeriod);
}
...
function renderMonthlyStockVolumeChart(period = 'year') {
    const ctx = document.getElementById('monthly-stock-volume-chart');
    if (!ctx) return;

    // Destroy existing chart instance
    if (monthlyStockVolumeChartInstance) {
        monthlyStockVolumeChartInstance.destroy();
    }

    // Get all stock transactions
    const transactions = loadTransactions();
    let stockTransactions = transactions.filter(tx => tx.assetType === 'stocks');

    // Filter by period if 'year'
    if (period === 'year') {
        const oneYearAgo = new Date();
        oneYearAgo.setFullYear(oneYearAgo.getFullYear() - 1);
        stockTransactions = stockTransactions.filter(tx => new Date(tx.date) >= oneYearAgo);
    }

    // Group transactions by month
    const monthlyData = groupTransactionsByMonth(stockTransactions, 'type');

    if (monthlyData.labels.length === 0) {
        ctx.parentElement.innerHTML = '<div class="text-center py-8 text-gray-400">No stock transactions yet</div>';
        return;
    }
    ...
    // Create the chart
    monthlyStockVolumeChartInstance = new Chart(ctx, {
        type: 'bar',
        data: {
